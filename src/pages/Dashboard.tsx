import { Package, DollarSign, AlertTriangle, ArrowUpDown, TrendingUp, ShoppingBag, Loader2 } from "lucide-react";
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-4">
        ...
        <MetricCard title="Total Value" value={`$${(metrics?.totalValue ?? 0).toLocaleString()}`} icon={DollarSign} iconColor="text-accent" delay={0.05} />
            ...
            {lowStockProducts.map((product) => (
              <div key={product.id} className="flex items-center justify-between p-3 rounded-lg bg-secondary/50">
                ...
                <span className={`text-xs font-semibold px-2.5 py-1 rounded-full ${
                  product.quantity === 0 ? "bg-destructive/20 text-destructive" : "bg-warning/20 text-warning"
          ...
          <div className="space-y-3">
            {recentTx.map((tx) => (
              <div key={tx.id} className="flex items-center justify-between p-3 rounded-lg bg-secondary/50">
                ...
                <span className={`text-xs font-semibold px-2.5 py-1 rounded-full ${
                  tx.transaction_type === "IN" ? "bg-accent/20 text-accent" : "bg-primary/20 text-primary"
